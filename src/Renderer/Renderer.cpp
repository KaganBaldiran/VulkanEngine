    ...
    if (PerformanceModeEnabledMeshCount)
    {
        ...
        if (rendererContext->DeviceContext.DeviceFeatures.multiDrawIndirect)
        {
            vkCmdDrawIndexedIndirect(
                ...
                sizeof(VKSCENE::ExtendedIndirectCommand)
            );
        }
        else
        {
            for (size_t i = 0; i < PerformanceModeEnabledMeshCount; i++)
            {
                vkCmdDrawIndexedIndirect(
                    CommandBuffer,
                    Scene.MeshBuffers.PerformanceModeBuffers.IndirectBuffers[CurrentFrame].Buffer.BufferObject,
                    i * sizeof(VKSCENE::ExtendedIndirectCommand),
                    1,
                    sizeof(VKSCENE::ExtendedIndirectCommand)
                );
            }
...
{
    VKCORE::ShaderModule LightingVertexShaderModule("Shaders\\LightingPassShader.vert", "Shaders\\LightingPassShaderVert.spv", VKCORE_GLOBAL_PREFERENCES_COMPILE_SHADERS, LogicalDevice);
    VKCORE::ShaderModule LightingFragmentShaderModule("Shaders\\LightingPassShader.frag", "Shaders\\LightingPassShaderFrag.spv", VKCORE_GLOBAL_PREFERENCES_COMPILE_SHADERS, LogicalDevice);
    VKCORE::ShaderModule PhysicsDebugVertexShaderModule("Shaders\\PhysicsDebugShader.vert", "Shaders\\PhysicsDebugShaderVert.spv", VKCORE_GLOBAL_PREFERENCES_COMPILE_SHADERS, LogicalDevice);
    VKCORE::ShaderModule PhysicsDebugFragmentShaderModule("Shaders\\PhysicsDebugShader.frag", "Shaders\\PhysicsDebugShaderFrag.spv", VKCORE_GLOBAL_PREFERENCES_COMPILE_SHADERS, LogicalDevice);
